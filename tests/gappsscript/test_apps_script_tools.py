...
from core.async_bridge import await_only
...
async def test_update_script_content_orders_concurrent_merges_for_same_script():
    ...
    # The fake client runs on the bridge like the real one, so the hand-offs
    # between the two in-flight updates are loop events awaited through it.
    first_update_started = asyncio.Event()
    allow_first_update = asyncio.Event()
    first_update_applied = asyncio.Event()
        ...
        def updateContent(self, scriptId, body):
            ...
            def execute():
                ...
                if update_number == 1:
                    ...
                    await_only(asyncio.wait_for(allow_first_update.wait(), 1))
                else:
                    await_only(asyncio.wait_for(first_update_applied.wait(), 1))
    ...
    await asyncio.wait_for(first_update_started.wait(), 1)
